"""Development generator and executable model for _mm_min_ps."""
...
CASE_ID = "sse2.min.f32x4.default"
...
def candidates(
    ...
    structured = (
        ((0x3F800000, 0x40400000, 0xC0000000, 0xC0800000),
         (0x40000000, 0x3F000000, 0xC0400000, 0xC0000000), "structured"),
        ((0x00000000, 0x80000000, 0x00000000, 0x80000000),
         (0x80000000, 0x00000000, 0x00000000, 0x80000000), "boundary"),
        ((0x7FC00042, 0x3F800000, 0xFFC00043, 0xBF800000),
         (0x40000000, 0x7FC01234, 0xC0400000, 0xFFC05678), "boundary"),
        ((0x7F800001, 0x3F800000, 0xFF800001, 0xBF800000),
         (0x40000000, 0x7F800001, 0xC0400000, 0xFF800001), "boundary"),
        ((0x7F800000, 0xFF800000, 0x7F7FFFFF, 0xFF7FFFFF),
         (0x7F7FFFFF, 0xFF7FFFFF, 0x7F800000, 0xFF800000), "boundary"),
        ((0x00000001, 0x80000001, 0x007FFFFF, 0x807FFFFF),
         (0x00000002, 0x80000002, 0x00800000, 0x80800000), "boundary"),
        ((0x3F800000, 0xBF800000, 0x4B000000, 0xCB000000),
         (0x3F800000, 0xBF800000, 0x4B000001, 0xCB000001), "boundary"),
        ((0x7FFFFFFF, 0xFFFFFFFF, 0x7FC00000, 0xFFC00000),
         (0x00000000, 0x80000000, 0x7F800000, 0xFF800000), "boundary"),
        ((0x3EAAAAAB, 0xBEAAAAAB, 0x3F000000, 0xBF000000),
         (0x3EAAAAAA, 0xBEAAAAAA, 0x3F000001, 0xBF000001), "boundary"),
        ((0x00800000, 0x80800000, 0x00000000, 0x80000000),
         (0x007FFFFF, 0x807FFFFF, 0x80000000, 0x00000000), "boundary"),
    )
...
def _minimum(a: int, b: int) -> int:
    if _is_nan(a) or _is_nan(b) or _from_bits(a) == _from_bits(b):
        return b
    return a if _from_bits(a) < _from_bits(b) else b
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    lanes = [
        f"0x{_minimum(int(str(a), 16), int(str(b), 16)):08x}"
