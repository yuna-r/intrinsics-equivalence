"""Development generator and executable model for _mm_packs_epi16."""
...
CASE_ID = "sse2.packs.i16x8.default"
...
def candidates(
    ...
    structured = (
        ((0, 1, 0xFFFF, 127, 128, 0xFF80, 0xFF7F, 0x8000), (0x7FFF, 126, 129, 0xFF81, 0xFF7E, 2, 0xFFFE, 0)),
        ((0x8000, 0x8001, 0xFFFF, 0, 1, 0x7FFE, 0x7FFF, 0x0080), (0xFF80, 0xFF81, 0x007E, 0x007F, 0x0080, 0x0081, 0x00FF, 0x0100)),
        ((0xAAAA,) * 8, (0x5555,) * 8),
        ((0, 0xFFFF) * 4, (127, 0xFF80) * 4),
    )
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    lanes = [f"0x{max(-128, min(127, value)) & 0xFF:02x}" for value in values]
