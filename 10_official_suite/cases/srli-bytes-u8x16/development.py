"""Development generator and executable model for _mm_srli_si128."""
...
CASE_ID = "sse2.srli-bytes.u8x16.imm8"
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    values = ([str(lane) for lane in lanes[count:]] + ["0x00"] * count) if count < 16 else ["0x00"] * 16
