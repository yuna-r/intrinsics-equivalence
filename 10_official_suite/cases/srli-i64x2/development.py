"""Development generator and executable model for _mm_srli_epi64."""
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    return {"return": vector("i64", tuple((int(str(lane), 16) >> count) if count < 64 else 0 for lane in lanes))}
