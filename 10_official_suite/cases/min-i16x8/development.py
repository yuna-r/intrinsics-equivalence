"""Development generator and executable model for _mm_min_epi16."""
...
CASE_ID = "sse2.min.i16x8.default"
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    values = tuple(min(_signed(str(x)), _signed(str(y))) for x, y in zip(left, right, strict=True))
