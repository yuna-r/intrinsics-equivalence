"""Development generator and executable model for _mm_shufflehi_epi16."""
...
CASE_ID = "sse2.shufflehi.i16x8.imm8"
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    result = [str(lane) for lane in lanes[:4]]
    result.extend(str(lanes[4 + ((control >> (2 * lane)) & 3)]) for lane in range(4))
