"""Development generator and executable model for _mm_adds_epu16."""
...
CASE_ID = "sse2.adds.u16x8.default"
...
def candidates(
    ...
    structured = (
        ((0, 1, 0xFFFF, 0xFFFE, 0x8000, 0x7FFF, 0x4000, 0xC000),
         (0, 1, 1, 2, 0x8000, 0x8000, 0xBFFF, 0x4000)),
        ((0xFFFF,) * 8, (0, 1, 0xFFFF, 0x8000, 0x7FFF, 2, 0x4000, 0xC000)),
        ((0xFFFE, 0xFFFD, 0x8000, 0x7FFF, 1, 2, 0x4000, 0xC000),
         (1, 2, 0x7FFF, 0x8000, 0xFFFE, 0xFFFD, 0xC000, 0x4000)),
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    values = [
        min(MASK16, (int(str(a), 16) & MASK16) + (int(str(b), 16) & MASK16))
