"""Development generator and executable model for _mm_unpackhi_epi8."""
...
CASE_ID = "sse2.unpackhi.i8x16.default"
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    values = tuple(value for index in range(8, 16) for value in (int(str(left[index]), 16), int(str(right[index]), 16)))
