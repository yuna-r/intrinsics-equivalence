"""Development generator and executable model for _mm_sad_epu8."""
...
CASE_ID = "sse2.sad.u8x16.default"
MINIMUM_COUNTS = {"standard": 4}
MASK = 0xFF
STRUCTURED = (
    ((0,) * 16, (0,) * 16),
    ((0,) * 8 + (0xFF,) * 8, (0xFF,) * 8 + (0,) * 8),
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    differences = [abs(int(str(x), 16) - int(str(y), 16)) for x, y in zip(left, right, strict=True)]
    return {"return": vector("u64", (sum(differences[:8]), sum(differences[8:])))}
