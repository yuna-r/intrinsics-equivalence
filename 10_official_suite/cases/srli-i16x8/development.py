"""Development generator and executable model for _mm_srli_epi16."""
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    return {"return": vector("i16", tuple((int(str(lane), 16) >> count) if count < 16 else 0 for lane in lanes))}
