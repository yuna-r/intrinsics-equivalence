"""Development generator and executable model for _mm_mulhi_epu16."""
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    return {"return": vector("u16", tuple((int(str(x), 16) * int(str(y), 16)) >> 16 for x, y in zip(left, right, strict=True)))}
