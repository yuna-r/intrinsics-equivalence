"""Development generator and executable model for _mm_subs_epi8."""
...
CASE_ID = "sse2.subs.i8x16.default"
...
def candidates(
    ...
    structured = (
        ((0x00,) * 16, (0x00,) * 16),
        ((0x7F, 0x7E, 0x40, 0x01) * 4, (0xFF, 0xFE, 0xC0, 0x80) * 4),
        ((0x80, 0x81, 0xC0, 0xFF) * 4, (0x01, 0x02, 0x40, 0x7F) * 4),
        ((0x7F, 0x80, 0xFF, 0x01) * 4, (0x80, 0x7F, 0x01, 0xFF) * 4),
        (tuple(range(16)), tuple(reversed(range(16)))),
        ((0x55, 0xAA, 0x33, 0xCC) * 4, (0xAA, 0x55, 0x99, 0x66) * 4),
    )
