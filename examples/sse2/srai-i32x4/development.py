"""Development generator and executable model for _mm_srai_epi32."""
...
def execute(record: dict[str, JSONValue]) -> dict[str, JSONValue]:
    ...
    count = min(int(immediates["imm8"]), 31)
    lanes = [f"0x{(_signed32(str(lane)) >> count) & MASK32:08x}" for lane in source_lanes]
