...
class EndToEndTests(unittest.TestCase):
    ...
    def test_one_command_development_check(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["record_count"], 256)
            ...
            self.assertIn("[7/7] PASS - 256 trials: done", progress)
            self.assertIn("Verification metrics", progress)
            self.assertRegex(progress, r"cases\s+128")
            self.assertRegex(progress, r"trials\s+256")
            self.assertRegex(progress, r"implementation-path evaluations\s+512")
            self.assertRegex(progress, r"lane verdicts\s+1,462")
    ...
    def test_check_prints_metrics_without_showcase_report(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertRegex(metrics, r"cases\s+128")
            self.assertRegex(metrics, r"trials\s+128")
            self.assertRegex(metrics, r"implementation-path evaluations\s+256")
            self.assertRegex(metrics, r"lane verdicts\s+731")
