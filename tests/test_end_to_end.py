...
class EndToEndTests(unittest.TestCase):
    ...
    def test_one_command_development_check(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["record_count"], 292)
            ...
            self.assertIn("[7/7] PASS - 292 trials: done", progress)
            self.assertIn("Verification metrics", progress)
            self.assertRegex(progress, r"cases\s+146")
            self.assertRegex(progress, r"trials\s+292")
            self.assertRegex(progress, r"implementation-path evaluations\s+584")
            self.assertRegex(progress, r"lane verdicts\s+1,584")
            self.assertRegex(progress, r"bit positions\s+35,136")
    ...
    def test_check_prints_metrics_without_showcase_report(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertRegex(metrics, r"cases\s+146")
            self.assertRegex(metrics, r"trials\s+146")
            self.assertRegex(metrics, r"implementation-path evaluations\s+292")
            self.assertRegex(metrics, r"lane verdicts\s+792")
            self.assertRegex(metrics, r"bit positions\s+17,568")
    ...
    def test_quality_option_is_an_eighth_opt_in_stage(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("[8/8] PASS - 146 trials", progress)
