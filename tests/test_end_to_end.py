...
class EndToEndTests(unittest.TestCase):
    ...
    def test_one_command_development_check(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result["record_count"], 508)
            ...
            self.assertIn("[7/7] PASS - 508 trials: done", progress)
            self.assertIn("Verification metrics", progress)
            self.assertRegex(progress, r"cases\s+254")
            self.assertRegex(progress, r"trials\s+508")
            self.assertRegex(progress, r"implementation-path evaluations\s+1,016")
            self.assertRegex(progress, r"lane verdicts\s+2,262")
            self.assertRegex(progress, r"bit positions\s+60,032")
            ...
            self.assertRegex(progress, r"valid contracts\s+254 / 254")
            self.assertRegex(progress, r"portable models\s+254 / 254")
            self.assertRegex(progress, r"architecture bindings\s+508 / 508")
    ...
    def test_check_prints_metrics_without_showcase_report(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertRegex(metrics, r"cases\s+254")
            self.assertRegex(metrics, r"trials\s+254")
            self.assertRegex(metrics, r"implementation-path evaluations\s+508")
            self.assertRegex(metrics, r"lane verdicts\s+1,131")
            self.assertRegex(metrics, r"bit positions\s+30,016")
